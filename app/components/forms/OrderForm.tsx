...
export function OrderForm({
  ...
  const activeMenuItems = menuItems.filter(
    (m) =>
      m.status === "active" ||
      (isEdit && initialValues?.menu_items?.includes(m.id)),
  );
  ...
  const toggleMenuItem = (id: number) => {
    setValues((prev) => {
      ...
      if (current.includes(id)) {
        return { ...prev, menu_items: current.filter((x) => x !== id) };
      }
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    if (!values.customer_name || !values.room || values.menu_items.length === 0) {
      return;
    }
    if (!isEdit) {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          >
            <option value="">
              {activeRooms.length === 0 ? "No active rooms" : "Select room"}
            </option>
        ...
        <div className="space-y-1 sm:col-span-2">
          ...
          <div className="relative" ref={menuRef}>
            <button
              ...
              className="flex min-h-[42px] w-full items-center justify-between rounded-lg border border-slate-200 px-3 py-2 text-left text-sm outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100"
            >
              ...
              <div className="absolute left-0 right-0 top-full z-10 mt-1 max-h-48 overflow-auto rounded-lg border border-slate-200 bg-white shadow-lg">
                {activeMenuItems.length === 0 ? (
                  ...
                  activeMenuItems.map((item) => (
                    <label
                      key={item.id}
                      className="flex cursor-pointer items-center gap-2 border-b border-slate-100 px-3 py-2 hover:bg-slate-50 last:border-0"
                    >
                      <input
                        type="checkbox"
                        checked={values.menu_items.includes(item.id)}
          ...
          {values.menu_items.length === 0 && (
            <p className="text-xs text-amber-600">
              Select at least one menu item. Only active items are shown.
            </p>
          )}
        </div>
        <div className="space-y-1">
      ...
      <div className="flex justify-end gap-2 pt-2">
        <button
          type="submit"
          disabled={submitting || values.menu_items.length === 0}

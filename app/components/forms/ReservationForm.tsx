...
const CalendarIcon = () => (
  <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className="h-4 w-4 text-slate-400 pointer-events-none">
...
export function ReservationForm({ rooms, existingReservations = [], initialValues, onSubmit, submitLabel = "Save", isEdit = false }: Props) {
  ...
  const checkAvailability = (): boolean => {
    ...
    const activeReservations = existingReservations.filter(
      (r) => r.room === values.room && r.status !== "Cancelled" && (isEdit ? r.id !== initialValues?.id : true),
    );
    const overlaps = activeReservations.some((r) =>
      datesOverlap(values.check_in, values.check_out, r.check_in, r.check_out),
    );
    if (overlaps) {
      setAvailabilityError("This room is not available for the selected dates. Please choose different dates or another room.");
      return false;
    }
    return true;
  };
  ...
  const handleSubmit = async (e: FormEvent) => {
    e.preventDefault();
    setAvailabilityError(null);
    if (!values.room || !values.customer_name || !values.email || !values.phone || !values.check_in || !values.check_out || !values.guests) return;
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          <input type="email" value={values.email} onChange={(e) => handleChange("email", e.target.value)} required className={inputClass} />
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-700">Phone</label>
          <input type="tel" value={values.phone} onChange={(e) => handleChange("phone", e.target.value)} placeholder="0771234567" required className={inputClass} />
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-700">Check-in</label>
          <div className="relative flex items-center">
            <span className="pointer-events-none absolute left-3"><CalendarIcon /></span>
            <input type="date" value={values.check_in} onChange={(e) => handleChange("check_in", e.target.value)} required
              className="w-full rounded-lg border border-slate-200 py-2 pl-10 pr-3 text-sm text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 [&::-webkit-calendar-picker-indicator]:cursor-pointer" />
          </div>
        </div>
        <div className="space-y-1">
          ...
          <div className="relative flex items-center">
            <span className="pointer-events-none absolute left-3"><CalendarIcon /></span>
            <input type="date" value={values.check_out} onChange={(e) => handleChange("check_out", e.target.value)} required min={values.check_in}
              className="w-full rounded-lg border border-slate-200 py-2 pl-10 pr-3 text-sm text-slate-900 outline-none focus:border-emerald-500 focus:ring-2 focus:ring-emerald-100 [&::-webkit-calendar-picker-indicator]:cursor-pointer" />
          </div>
        </div>
        <div className="space-y-1">
          <label className="text-xs font-medium text-slate-700">Guests</label>
          <input type="number" min={1} value={values.guests} onChange={(e) => handleChange("guests", Number(e.target.value) || 1)} required className={inputClass} />
        </div>
        <div className="space-y-1">
      ...
      {availabilityError && (
        <div className="rounded-lg bg-rose-50 px-3 py-2 text-sm text-rose-700">{availabilityError}</div>
      )}

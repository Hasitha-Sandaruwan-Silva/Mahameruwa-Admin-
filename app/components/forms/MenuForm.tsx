...
export function MenuForm({ initialValues, onSubmit, onCancel, submitLabel = "Save" }: Props) {
  ...
  const handleChange = (field: keyof MenuFormValues, value: string | number) => {
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid gap-4 sm:grid-cols-2">
        ...
        <div className="space-y-1">
          ...
          <input type="number" value={values.price} onChange={(e) => handleChange("price", Number(e.target.value ?? 0))} min={0} required className={inputClass} />
        </div>
        <div className="space-y-1">
      ...
      <div className="flex justify-end gap-2 pt-2">
        {onCancel && (
          <button type="button" onClick={onCancel} className="rounded-full border border-slate-200 px-4 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50">
            Cancel
          </button>
        )}
